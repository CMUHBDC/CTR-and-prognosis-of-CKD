...
by _imp;
class &name6. gender(ref="0");
model &name2.*&name3.(0)=&name5. age_entry gender eGFR_CKDEPI PCRtotal_log/risklimits; 
...
proc phreg data=&name1.;
by _imp;
class &name6. gender(ref="0") havedm(ref='0') havehtn(ref='0') Amemia(ref='0');
model &name2.*&name3.(0)=&name5. age_entry gender havedm havehtn Amemia eGFR_CKDEPI/ risklimits; 
...
proc phreg data=&name1.;
by _imp;
class &name5. gender(ref="0");
model &name2.*&name3.(0)=&name4. age_entry gender eGFR_CKDEPI PCRtotal_log/ risklimits;
...
proc phreg data=&name1.;
by _imp;
class &name5. gender(ref="0") havedm(ref='0') havehtn(ref='0') Amemia(ref='0');
model &name2.*&name3.(0)=&name4. age_entry gender havedm havehtn Amemia eGFR_CKDEPI/ risklimits; 
...
by _imp;
class  &name2. gender(ref="0");
model dialysistime*event(0)=&name1. age_entry gender eGFR_CKDEPI PCRtotal_log;
...
proc phreg data=esrd.Ctrgroupfollowup_mi concordance=uno(se seed=1234 iter=100);
by _imp;
class  &name4. gender(ref="0") havedm(ref='0') havehtn(ref='0') Amemia(ref='0');
model &name1.*&name2.(0)=&name3. age_entry gender havedm havehtn Amemia eGFR_CKDEPI;
ods output  ParameterEstimates=param_origi  Concordance=cstatistic_origi;
run;
...
proc phreg data=esrd.Ctrgroupfollowup_mi concordance=uno (diff se seed=1234 iter=100);
    by _imp;
    class CTRgroup_New (ref="1") gender(ref="0") &name4.;
    model &name1.*&name2.(0)=CCTR CTRgroup_New age_entry gender &name3./nofit;
    roc "&name5." &name6. age_entry gender &name3.;
    roc "&name5.+baseline CTR" &name7. age_entry gender &name3.;
    roc "&name5.+baseline CTR group" &name8. age_entry gender &name3.;
